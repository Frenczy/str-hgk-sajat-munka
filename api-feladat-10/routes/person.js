...
/* Returns one User. */
router.get('/:id', authenticateJwt, async function (request, response, next) {
...
router.get('/:id/vaccinated', authenticateJwt, async function (request, response, next) {
...
/* Records one user */
router.post('/', authenticateJwt, adminOnly, async function (request, response, next) {
...
/* GET person count */
router.get('/count', authenticateJwt, async function (req, res, next) {
    const people = await Person.find()
